...
public class ApiKeySortBy implements AbstractFacade.SortBy {
  
  private final ApiKeyFacade.Sorts sortBy;
  private final AbstractFacade.OrderBy param;
  
  public ApiKeySortBy(String param) {
    String[] sortByParams = param.split(":");
    String sort = "";
    try {
      sort = sortByParams[0].toUpperCase();
      this.sortBy = ApiKeyFacade.Sorts.valueOf(sort);
