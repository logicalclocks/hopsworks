...
public class FilterBy implements AbstractFacade.FilterBy {
  private final TagSchemasFacade.Filters filter;
  private final String param;
  
  public FilterBy(String param) {
    if (param.contains(":")) {
      this.filter = TagSchemasFacade.Filters.valueOf(param.substring(0, param.indexOf(':')).toUpperCase());
      this.param = param.substring(param.indexOf(':') + 1);
    } else {
      this.filter = TagSchemasFacade.Filters.valueOf(param);
