...
public class ProjectAlertsSortBy implements AbstractFacade.SortBy {
  
  private final ProjectServiceAlertsFacade.Sorts sortBy;
  private final AbstractFacade.OrderBy param;
  
  public ProjectAlertsSortBy(String param) {
    String[] sortByParams = param.split(":");
    String sort = "";
    try {
      sort = sortByParams[0].toUpperCase();
      this.sortBy = ProjectServiceAlertsFacade.Sorts.valueOf(sort);
