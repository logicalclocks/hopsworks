...
public class FeatureMonitoringResultSortBy implements AbstractFacade.SortBy {
  private final FeatureMonitoringResultFacade.Sorts sortBy;
  private final AbstractFacade.OrderBy param;
  
  public FeatureMonitoringResultSortBy(String param) {
    String[] sortByParams = param.split(":");
    String sort = "";
    try {
      sort = sortByParams[0].toUpperCase();
      this.sortBy = FeatureMonitoringResultFacade.Sorts.valueOf(sort);
    ...
    try {
      ...
      throw new WebApplicationException(
        "Sort by " + sort + " needs to set a valid order(asc|desc), but found: " + order, Response.Status.NOT_FOUND);
    }
  }
