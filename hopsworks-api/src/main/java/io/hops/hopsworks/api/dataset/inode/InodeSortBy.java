...
public class InodeSortBy implements AbstractFacade.SortBy {
  
  private final InodeFacade.Sorts sortBy;
  private final AbstractFacade.OrderBy param;
  
  public InodeSortBy(String param) {
    String[] sortByParams = param.split(":");
    String sort = "";
    try {
      sort = sortByParams[0].toUpperCase();
      this.sortBy = InodeFacade.Sorts.valueOf(sort);
