...
public class DatasetSharedWithSortBy implements AbstractFacade.SortBy {
  
  private final DatasetSharedWithFacade.Sorts sortBy;
  private final AbstractFacade.OrderBy param;
  
  public DatasetSharedWithSortBy(String param) {
    String[] sortByParams = param.split(":");
    String sort = "";
    try {
      sort = sortByParams[0].toUpperCase();
      this.sortBy = DatasetSharedWithFacade.Sorts.valueOf(sort);
