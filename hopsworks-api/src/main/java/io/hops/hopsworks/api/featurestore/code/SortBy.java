...
public class SortBy implements AbstractFacade.SortBy {
  ...
  private final FeaturestoreCodeFacade.Sorts sortBy;
  ...
  public SortBy(String param) {
    ...
    try {
      ...
      this.sortBy = FeaturestoreCodeFacade.Sorts.valueOf(sort);
    ...
    try {
      ...
      throw new WebApplicationException("Sort by " + sort +
              " need to set a valid order(asc|desc), but found: " + order,
              Response.Status.NOT_FOUND);
    }
  }
