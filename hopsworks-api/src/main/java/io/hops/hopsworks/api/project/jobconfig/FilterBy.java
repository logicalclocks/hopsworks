...
public class FilterBy implements AbstractFacade.FilterBy {
  ...
  private final DefaultJobConfigurationFacade.Filters filter;
  private final String param;

  public FilterBy(String param) {
    if (param.contains(":")) {
      this.filter = DefaultJobConfigurationFacade.Filters.valueOf(param.substring(0, param.indexOf(':')).toUpperCase());
      this.param = param.substring(param.indexOf(':') + 1);
    } else {
      this.filter = DefaultJobConfigurationFacade.Filters.valueOf(param);
