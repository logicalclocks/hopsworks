...
public class SchemalessMetadata implements Serializable {
  ...
  @JoinColumns({
    @JoinColumn(name = "inode_parent_id", referencedColumnName = "parent_id", 
            insertable = false,
            updatable = false),
    @JoinColumn(name = "inode_name", referencedColumnName = "name"),
    @JoinColumn(name = "inode_partition_id", referencedColumnName = "partition_id")
  })
  ...
  public SchemalessMetadata(Inode inode) {
    this.pk = new SchemalessMetadataPK(inode.getId(), inode.getInodePK().getParentId());
    this.inode = inode;
  }
