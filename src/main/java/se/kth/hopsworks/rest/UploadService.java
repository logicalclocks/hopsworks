...
public class UploadService {
   ...
   * @throws AppException if there is a folder name that is not valid in the given path, the path is empty, or project
   * name was not found.
   */
  ...
  /**
   * Sets the path for the file to be uploaded. It does not require a project name since the file to be uploaded is a
   * template schema, irrelevant to any project or dataset. The only requirement is that the upload has to be performed
...
//        dfsOps.copyToHDFSFromLocal(true, stagingManager.
        ...
        fileOps.copyToHDFSFromLocal(true, stagingManager.
            getStagingPath()
            + this.path + fileName, "hdfs://" + hdfsLeDescriptors.getHostname() + this.path
            + fileName);
...
//        dfsOps.setPermission(location, dfsOps.getParentPermission(location)); 
        dfsOps.setOwner(location, username, username);
