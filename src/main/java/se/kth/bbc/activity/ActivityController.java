...
public class ActivityController {
   ...
   * @return
   */
  public List<ActivityDetail> activityDetailOnStudyAudit(String studyName, Date from,
          Date to) {
    String sql =null;
    if(!studyName.isEmpty() && studyName!=null){
      sql = "SELECT * FROM activity_details  WHERE  (created >= '"
            + from + "' AND created <='" + to + "' AND studyname ='"
            + studyName + "')";
    }else {
      sql= "SELECT * FROM activity_details  WHERE  (created >= '"
            + from + "' AND created <='" + to + "')";
    ...
    Query query = em.createNativeQuery(sql, ActivityDetail.class);

    List<ActivityDetail> ul = query.getResultList();
