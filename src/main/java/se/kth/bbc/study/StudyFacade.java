...
public class StudyFacade extends AbstractFacade<TrackStudy> {
  ...
  public List<TrackStudy> findAllStudies(String user) {
    ...
    Query query = em.createNativeQuery(
            "SELECT name, username FROM study WHERE username=? UNION SELECT name, username FROM study WHERE name IN (SELECT name FROM StudyTeam WHERE team_member=?)",
            TrackStudy.class)
  ...
  public List<StudyDetail> findAllStudyDetails(String useremail) {
    Query query = em.createNativeQuery(
            "SELECT * FROM StudyDetails WHERE studyName IN (SELECT name FROM StudyTeam WHERE team_member=?)",
            StudyDetail.class)
  ...
  public List<TrackStudy> findJoinedStudies(String user) {
    ...
    Query query = em.createNativeQuery(
            "select study.name, study.username from (StudyTeam join study on StudyTeam.name=study.name) join USERS on study.username=USERS.email where study.username not like ? and StudyTeam.team_member like ?",
            TrackStudy.class)
            .setParameter(1, user).setParameter(2, user);
    return query.getResultList();
  }
  ...
  public List<StudyDetail> findJoinedStudyDetails(String useremail) {
    Query query = em.createNativeQuery(
            "SELECT * FROM StudyDetails WHERE studyName IN (SELECT name FROM StudyTeam WHERE team_member=?) AND email NOT LIKE ?",
            StudyDetail.class)
  ...
  public List<TrackStudy> QueryForNonRegistered(String user) {
    ...
    Query query = em.createNativeQuery(
            "SELECT name, username FROM study WHERE name IN (SELECT name FROM StudyTeam WHERE team_member=?)",
            TrackStudy.class)
            .setParameter(1, user);
    return query.getResultList();
  }
