 ...
 * each running job, its CancellableJob object is registered. The RunningJobTracker
...
public class RunningJobTracker {
  ...
  private static final String MSG_JOB_ALREADY_PRESENT = "A job with this ID has already been registered.";
  private static final String MSG_JOB_NOT_RUNNING = "Attempting to unregister job with non-existing id.";
  
  private final ConcurrentMap<Long,CancellableJob> runningJobs = new ConcurrentHashMap<>();
  
  //TODO: use different exception or return value?
  public void registerJob(Long jobId, CancellableJob job) throws IllegalStateException{
