...
public class JobFacade extends AbstractFacade<Job> {
   ...
   * @param execFiles The set of execution files for the job (jars,
   * workflows,...)
  ...
  public Job create(String name, Users user, Project project, JobType type,
          Collection<JobExecutionFile> execFiles, YarnJobConfiguration config) {
    //First: create a job object
    Job job = new Job(type, config, project, user, name, execFiles);
