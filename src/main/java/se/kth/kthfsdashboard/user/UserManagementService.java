...
public class UserManagementService {
  ...
  public Response login(@FormParam("email") String email,
    ...
    if (req.getUserPrincipal() == null) {
      try {
        ...
        req.getServletContext().log(
                "Authentication Demo: successfully logged in " + email);
    ...
    req.getServletContext().log(
            "Authentication Demo: successfully retrieved User Profile from DB for "
            + email);
    json.setData(user);
  ...
  public Response register(UserDTO newUser, @Context HttpServletRequest req) {
    ...
    req.getServletContext().log("successfully registered new user: '" + newUser.
            getEmail() + "':'" + newUser.getPassword1() + "'");

    req.getServletContext().log("execute login now: '" + newUser.getEmail()
    ...
    try {
      ...
      e.printStackTrace();
      json.setErrorMsg(
              "User Account created, but login failed. Please try again later.");
