...
public class AuthBackingBean {
  ...
  public String login() {
    ...
    if (user != null && user.getStatus() == Username.STATUS_REQUEST) {
      FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Fail",
    ...
    if (request.isUserInRole("BBC_ADMIN") || request.isUserInRole(
            "BBC_RESEARCHER") || request.isUserInRole("ADMIN") || request.
            isUserInRole("USER")) {
...
//      TODO does not work correctly
    ...
    log.log(Level.INFO, "Logging OUT Authenticated user: {0}", principal.
            getName());
    ...
    if (s != null) {
      try {
        ...
        log.log(Level.SEVERE, "Throwable Exception when calling logout user! ",
                e.toString());
      }
    }
    return result;
