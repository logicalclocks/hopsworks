...
public class UserFacade extends AbstractFacade<Username> {
  ...
  public List<Username> findAll() {
    TypedQuery<Username> query = em.createNamedQuery("Username.findAll",
            Username.class);
    return query.getResultList();
  }

  public List<Username> findAllByName() {
    TypedQuery<Username> query = em.createNamedQuery("Username.findAllByName",
            Username.class);
    return query.getResultList();
  }
...
//        return themes;
  ...
  public List<Username> findAllByStatus(int status) {
    TypedQuery<Username> query = em.createNamedQuery("Username.findAllByStatus",
            Username.class);
