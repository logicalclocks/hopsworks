...
module OpensearchHelper
  ...
  def opensearch_status_details(response, expected_status, error_type: nil)
  ...
  def opensearch_nodes_ids()
    path = "_cat/nodes"
    query = "format=JSON&full_id=true&v=true&h=id"
    opensearch_rest do
      response = opensearch_get "#{path}?#{query}"
      opensearch_status_details(response, 200)
      JSON.parse(response.body)
    end
  end

  def opensearch_nodes_stats(node_id)
    path = "_nodes/stats"
    opensearch_rest do
      response = opensearch_get path
      opensearch_status_details(response, 200)
