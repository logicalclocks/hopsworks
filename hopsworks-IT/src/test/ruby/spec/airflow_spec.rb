...
describe "On #{ENV['OS']}" do
  ...
  before :all do
    reset_session
    @dag_name = "test_dag"
    @dag_definition = {"projectId":"126","name":"#{@dag_name}","owner":"meb10000","scheduleInterval":"@once", "operators":[{"HAS_JOB_NAME_ATTR":0,"HAS_WAIT_2_FINISH_ATTR":1, "HAS_FEATURE_GROUPS_NAME_ATTR":2,"name":"HopsworksLaunchOperator", "description":"Operator to launch a Job in Hopsworks. Job should already bedefined in Jobs UI and job name in operator must match the job name in Jobs UI.","attributesMask":[true,true], "wait":true,"id":"launch_job0","jobName":"Job0","dependsOn":[], "jobArgs":""}, {"HAS_JOB_NAME_ATTR":0,"HAS_WAIT_2_FINISH_ATTR":1, "HAS_FEATURE_GROUPS_NAME_ATTR":2,"name":"HopsworksLaunchOperator", "description":"Operator to launch a Job in Hopsworks. Job should already bedefined in Jobs UI and job name in operator must match the job name in Jobs UI.","attributesMask":[true,true], "wait":false,"id":"launch_job1","jobName":"Job1","dependsOn":["launch_job0"], "jobArgs":"3"},{"HAS_JOB_NAME_ATTR":0,"HAS_WAIT_2_FINISH_ATTR":1, "HAS_FEATURE_GROUPS_NAME_ATTR":2,"name":"HopsworksJobSuccessSensor", "description":"Operator which waits for the completion of aspecific job. Job must be defined in Jobs UI and job name in operator must match the job name in Jobs UI. The task will fail too if the job which is waiting for fails.","attributesMask":[true,false],"id":"wait_job1", "jobName":"Job1","dependsOn":["launch_job1"]}, {"HAS_JOB_NAME_ATTR":0,"HAS_WAIT_2_FINISH_ATTR":1, "HAS_FEATURE_GROUPS_NAME_ATTR":2,"name":"HopsworksLaunchOperator", "description":"Operator to launch a Job in Hopsworks. Job should already be defined in Jobs UI and job name in operator must match the job name in Jobs UI.","attributesMask":[true,true], "wait":true,"id":"launch_job2","jobName":"Job2","dependsOn":["launch_job0", "wait_job1"], "jobArgs":"3"},{"HAS_JOB_NAME_ATTR":0,"HAS_WAIT_2_FINISH_ATTR":1, "HAS_FEATURE_GROUPS_NAME_ATTR":2,"name":"HopsworksFeatureValidationResult", "description":"When this task runs, it will fetch the Data Validation result for a specific feature group. It assumes that the data validation Job has run before and it has finished. The task will fail if the validation result is not successful.","attributesMask":[false,false,true],"id":"fetch_dv","featureGroupName":"some_feature_group","dependsOn":["launch_job2"]}]}
  end
