...
describe "On #{ENV['OS']}" do
  ...
  describe 'model export' do
    ...
    context 'as python jobs' do
      before(:all) do
        if ENV['OS'] == "ubuntu"
          skip "python jobs do not run on non kube (ubuntu)"
        end
      end
      it 'base' do
        ...
        job_type = "PYTHON"
        run_job_check_partials(@user1_params, @project1, job_name, job_type, file_type,
                               get_python_default_config(@project1, job_name, file_type)) do
          model_exists(@project1, "mnist_python", version: 1)
        end
      end
    end
  end
