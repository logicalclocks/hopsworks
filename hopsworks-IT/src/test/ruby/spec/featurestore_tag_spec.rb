    ...
    context "with no valid tags" do
      context "featuregroups" do
        ...
        it "should not be able to attach tags which are not defined" do
            ...
            json_result, featuregroup_name = create_cached_featuregroup(project.id, featurestore_id)
            parsed_json = JSON.parse(json_result)
            add_featuregroup_tag(project.id, featurestore_id, parsed_json["id"], "doesnotexist", "abc123")
            expect_status(400)
        end
      ...
      context "training datasets" do
        ...
        it "should not be able to attach tags which are not defined" do
            ...
            json_result, training_dataset_name = create_hopsfs_training_dataset(project.id, featurestore_id, connector)
            parsed_json = JSON.parse(json_result)
            add_training_dataset_tag(project.id, featurestore_id, parsed_json["id"], "doesnotexist", "abc123")
            expect_status(400)
        end
      end
    end
    ...
    context "with valid tags" do
      ...
      context "featuregroups" do
        ...
        it "should not be able to get tag which is not attached" do
            ...
            add_featuregroup_tag(project.id, featurestore_id, fg_json["id"], "updated", "daily")
            expect_status(201)
        ...
        it "should be able to delete attached tags" do
            ...
            json_result, featuregroup_name = create_cached_featuregroup(project.id, featurestore_id)
            fg_json = JSON.parse(json_result)
            add_featuregroup_tag(project.id, featurestore_id, fg_json["id"], "jobtype", "batch")
            expect_status(201)
      ...
      context "training datasets" do
        ...
        it "should not be able to get tag which is not attached" do
            ...
            add_training_dataset_tag(project.id, featurestore_id, td_json["id"], "updated", "daily")
            expect_status(201)
