...
echo '
{
	...
	"elasticsearch.cluster" : "hopsworks",
		...
		{
		"statement": "SELECT ops.inode_id as _id, ops.dataset_id as _parent, ops.* FROM hops.hdfs_metadata_log ops WHERE ops.operation = 1 AND ops.inode_id IN (SELECT inode_id FROM hopsworks.meta_inodes_ops_datasets_deleted WHERE processed = 0)"
		},
