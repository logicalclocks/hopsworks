...
-- PARENT IS THE DATASET AND IS ASSUMED TO RESIDE IN METADATA_LOGS. IF THE PARENT DATASET IS NOT PRESENT IN THE LOGS TABLE
...
INSERT INTO hopsworks.meta_inodes_ops_children_ds_buffer (inodeid, parentid, operation, logical_time, processed) (

SELECT DISTINCT log.inode_id, log.dataset_id, log.operation, log.logical_time, 0 as processed
...
-- THE PARENT DATASET IS NOT PRESENT IN METADATA_LOG TABLE (THE CASE WHERE THE INODE WAS CREATED AFTER THE DATASET HAD BEEN INDEXED)
...
FROM hops.hdfs_metadata_log log, 
	...
	(SELECT p.id as datasetid, p.parent_id as parentt, p.* 
...
-- SELECT ALL CHILDREN (no datasets) THAT HAVE BEEN ADDED (OPERATION 0 - DATASET IS THE PARENT - indexes subdirs under the dataset)
...
FROM (
	...
	FROM hops.hdfs_inodes hi,  
		...
		FROM hopsworks.meta_inodes_ops_children_ds_buffer log
...
FROM hopsworks.meta_inodes_ops_children_ds_buffer c
WHERE c.operation = 1 LIMIT 100;


-- MARK AS PROCESSED ALL CHILDREN THAT HAVE BEEN PROCESSED (PROCESSED = 1 - deprecated)

UPDATE hopsworks.meta_inodes_ops_children_ds_buffer b SET b.processed = 1;


-- DELETE ALL PROCESSED CHILDREN FROM THE HDFS_METADATA_LOG TABLE

DELETE FROM hops.hdfs_metadata_log WHERE inode_id IN (SELECT inodeid FROM hopsworks.meta_inodes_ops_children_ds_buffer WHERE processed = 1);


-- DELETE ALL CHILDREN THAT HAVE BEEN MARKED AS PROCESSED (PROCESSED = 1)

DELETE FROM hopsworks.meta_inodes_ops_children_ds_buffer WHERE processed = 1;
