        ...
        .controller('JupyterCtrl', ['$scope', '$routeParams','$route',
          ...
          function ($scope, $routeParams, $route, growl, ModalService, JupyterService, $location) {
            ...
            var getNotesInProject = function (loading, reload) {
              ...
              if (reload) {
                JupyterService.reloadedNotebooks().then(function (success) {
                ...
                JupyterService.notebooks().then(function (success) {
                  self.notes = success.data.body;
                  self.notesRefreshing = false;
                  stopLoading();
                }, function (error) {
            ...
            var init = function () {
              ...
              JupyterService.websocket().sendNewEvent({op: 'LIST_NOTES'});
              self.connectedStatus = JupyterService.websocket().isConnected();
              $scope.tgState = true;
            };
            ...
            self.stopInterpreter = function (interpreter) {
              if (!interpreter.interpreter.notRunning) {
                ...
                JupyterService.restartInterpreter(interpreter.interpreter.id)
            ...
            self.stopLivySession = function (interpreter, sessionId) {
                self.transition = true;
                interpreter.statusMsg = statusMsgs[2];
                JupyterService.stopLivySession(interpreter.interpreter.id, sessionId)
                        .then(function (success) {
                          interpreter.interpreter = success.data.body;
                          interpreter.statusMsg = statusMsgs[(success.data.body.notRunning ? 0 : 1)];
                          self.transition = false;
            ...
            self.createNewNote = function () {
              ...
              ModalService.noteName('md', '', '', '').then(
                      function (success) {
                        ...
                        JupyterService.createNotebook(noteName).then(function (success) {
            ...
            self.deleteNote = function (note) {
              JupyterService.deleteNotebook(note.id).then(function (success) {                
            ...
            self.clearCache = function () {
              ...
              JupyterService.restart().then( function (success) {
                  //stopLoading();
                  $route.reload();
                }, function (error) {
                  stopLoading();
            ...
            JupyterService.websocket().ws.onMessage(function (event) {
              var payload;
              if (event.data) {
            ...
            JupyterService.websocket().ws.onOpen(function () {
              console.log('Websocket created');
            });

            JupyterService.websocket().ws.onError(function (event) {
              console.log('error message: ', event);
              self.connectedStatus = false;
            });

            JupyterService.websocket().ws.onClose(function (event) {
              self.connectedStatus = false;
