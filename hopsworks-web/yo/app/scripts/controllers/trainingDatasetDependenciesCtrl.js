...
/**
 * Controller for the training dataset-dependencies-view
 */
angular.module('hopsWorksApp')
    .controller('trainingDatasetDependenciesCtrl', ['$uibModalInstance', '$scope', 'FeaturestoreService', 'growl', 'projectId', 'trainingDataset',
        function ($uibModalInstance, $scope, FeaturestoreService, growl, projectId, trainingDataset) {
            ...
            self.setupDependencyChart = function () {
                ...
                rows.push(
                {"c": [
                    {"v": index, "f": self.trainingDataset.name},
                    {"v": ""}
                ]})
                index = index+1
                var i
                for (i = 0; i < self.trainingDataset.dependencies.length; i++) {
                    rows.push(
                        {"c": [
                            {"v": index, "f": self.trainingDataset.dependencies[i].path},
                            {"v": 1}
                        ]}
                    )
                    index = index+1
