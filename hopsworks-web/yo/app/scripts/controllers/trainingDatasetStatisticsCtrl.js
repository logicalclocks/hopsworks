...
angular.module('hopsWorksApp')
    .controller('trainingDatasetStatisticsCtrl', ['$uibModalInstance', '$scope', 'FeaturestoreService', 'ModalService',
        ...
        function ($uibModalInstance, $scope, FeaturestoreService, ModalService, growl, projectId,
            ...
            if('descriptiveStatistics' in trainingDataset){
                self.stats = trainingDataset.descriptiveStatistics
                self.preProcessStats()
            } else {
                self.stats = []
                self.preprocessedStats = []
            }
            if('featureCorrelationMatrix' in trainingDataset){
                self.featureMatrix = trainingDataset.featureCorrelationMatrix
            } else {
                self.featureMatrix = null
            }
            if('featuresHistogram' in trainingDataset){
                self.featureHistograms = trainingDataset.featuresHistogram
            } else {
                self.featureHistograms = []
            }
            if('clusterAnalysis' in trainingDataset){
                self.clusterAnalysis = trainingDataset.clusterAnalysis
            } else {
            ...
            self.setupHeatmap = function () {
                ...
                var heatmapConfig = {
                    ...
                    series:  series,
                    title: {
                        text: "Feature correlation for training dataset: " + self.trainingDataset.name,
                        align: 'center',
                        margin: 10,
                        offsetX: 0,
                        offsetY: 0,
            ...
            self.setupClusterAnalysis = function () {
                var clusterAnalysisPlotOption = {
                    ...
                    title: {
                        enable: true,
                        text: 'Cluster analysis for training dataset: ' + self.trainingDataset.name},
                    css: {
                        'text-align': 'center',
                        'color': 'black'
                    }
            ...
            self.showDescriptiveStatistics = function(){
                var statisticType= 0
                ModalService.viewFeaturestoreStatistic('lg', self.projectId, trainingDataset, statisticType, self.preProcessedStats, true).then(
                    function (success) {
                    }, function (error) {
                    });
            }
            ...
            self.showCorrelationMatrix = function(){
                var statisticType= 1
                ModalService.viewFeaturestoreStatistic('lg', self.projectId, trainingDataset, statisticType, self.heatmapOptions, true).then(
                    function (success) {
                    }, function (error) {
                    });
            }
