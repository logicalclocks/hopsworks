...
public abstract class RESTException extends Exception implements Serializable {
  ...
  public JsonResponse buildJsonResponse(JsonResponse jsonResponse, RESTLogLevel logLevel) {
    ...
    if (logLevel.getLevel() <= RESTLogLevel.PROD.getLevel()) {
      jsonResponse.setUsrMsg(usrMsg);
    }
    if (logLevel.getLevel() <= RESTLogLevel.TEST.getLevel()) {
      jsonResponse.setDevMsg(devMsg);
    }
    if (logLevel.getLevel() <= RESTLogLevel.DEV.getLevel()) {
