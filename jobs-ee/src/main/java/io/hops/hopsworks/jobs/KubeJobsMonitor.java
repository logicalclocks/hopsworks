      ...
      for (Pod pod : pods) {
        ...
        if (executionFacade.findById(executionId).isPresent()) {
          ...
          if (execution.getState() == JobState.FINISHED
            ...
            // If input file is jupyter notebook, the generated output notebook needs to be
            // in the execution table to be displayed in the UI. If no output is generated, path
            // must be hidden
            try {
              datasetController.checkFileExists(new org.apache.hadoop.fs.Path(execution.getNotebookOutPath()),
                execution.getHdfsUser());
            } catch (DatasetException e) {
              execution.setNotebookOutPath(null);
