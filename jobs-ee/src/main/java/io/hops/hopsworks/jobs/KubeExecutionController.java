...
public class KubeExecutionController extends AbstractExecutionController implements ExecutionController {
  ...
  public Execution start(Jobs job, String args, Users user) throws JobException, GenericException,
    ...
    if (job.getJobType() == JobType.PYTHON || job.getJobType() == JobType.DOCKER) {
      ...
      Execution execution = executionFacade.create(job, user, null, null, null, null, 0, hdfsUser, args);
      ...
      if (appPath.endsWith(".ipynb")) {
        // If a notebook is given as input, papermill will output here the result. Will be then displayed in the UI
        String fileNameWithOutExt = FilenameUtils.getBaseName(appPath);
        String notebookOutPath = logPath + "/" +fileNameWithOutExt + "-output.ipynb";
        execution.setNotebookOutPath(notebookOutPath);
      }
