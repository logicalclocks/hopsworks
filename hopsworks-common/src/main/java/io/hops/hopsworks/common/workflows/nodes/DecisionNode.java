...
public class DecisionNode extends Node {
  ...
  public Node getDefaultNode() {
    ...
    for (Edge edge : this.getOutEdges()) {
      if (edge.getType().equals("default-decision-edge")) {
        defaultNode = edge.getTarget();
      }
    }
    return defaultNode;
  }
  ...
  public Element getWorkflowElement(OozieFacade execution, Element root) throws
    ...
    for (Node node : this.getChildren()) {
      ...
      if (node.equals(defaultNode)) {
        continue;
      }
      if (node.getDecision() == null) {
        throw new ProcessingException("Decision should not be empty");
      }
