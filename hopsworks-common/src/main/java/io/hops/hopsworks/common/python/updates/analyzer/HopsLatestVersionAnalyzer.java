...
/*
  ...
  2.1.0.0 where the Hopsworks installation number is the first three digits separated by a dot.
  ...
  For each bugfix release the bugfix version is bumped by + 1, such as 2.1.0.0 -> 2.1.0.1 -> 2.1.0.2.
 */
public class HopsLatestVersionAnalyzer extends LatestVersionAnalyzer {

  private static final Logger LOGGER = Logger.getLogger(
    HopsLatestVersionAnalyzer.class.getName());
  ...
  public void setLatestVersion(String currentHopsworksVersion, HashMap<String, List<LibraryVersionDTO>> versions) {
    ...
    if(versions.containsKey(this.getLibrary()) && !versions.get(this.getLibrary()).isEmpty()) {
      ...
      List<LibraryVersionDTO> currentVersionSearchHits = searchVersionHits
        .stream().filter(version -> version.getVersion().startsWith(currentHopsworksVersion))
