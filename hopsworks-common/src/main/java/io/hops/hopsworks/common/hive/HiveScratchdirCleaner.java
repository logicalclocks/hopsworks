...
public class HiveScratchdirCleaner {
  ...
  @PostConstruct
  private void init() {
    applicationTypeSet = new HashSet<>();
    applicationTypeSet.add("TEZ");

    applicationStateEnumSet = EnumSet.of(YarnApplicationState.NEW, YarnApplicationState.GENERATING_CERTS,
  ...
  public void doCleanUp() {
    YarnClientWrapper yarnClientWrapper = yarnService.getYarnClientSuper(settings.getConfiguration());
        ...
        if (scratchDir.getModificationTime().longValue() < threshold){
          ...
          try {
            ...
            if (appReports.isEmpty()) {
              // No Hive query is running for this user. Safe to delete.
