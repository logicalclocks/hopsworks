...
public class KibanaClient {
  ...
  private PoolingHttpClientConnectionManager createConnectionManager() throws IOException, GeneralSecurityException {
    ...
    connectionManager.setMaxTotal(settings.OPENSEARCH_KIBANA_NO_CONNECTIONS);
    connectionManager.setDefaultMaxPerRoute(settings.OPENSEARCH_KIBANA_NO_CONNECTIONS);
    return connectionManager;
  }
  
  public JSONObject createIndexPattern(Users user, Project project,
      KibanaType type, String id) throws OpenSearchException {
  ...
  public JSONObject postWithOverwrite(Users user, Project project,
      KibanaType type, String id, String data) throws OpenSearchException {
  ...
  private JSONObject execute(HttpMethod method, KibanaType type, String id,
      String data, Users user, Project project) throws OpenSearchException {
  ...
  private JSONObject execute(HttpMethod method, KibanaType type, String id,
    ...
    try {
      ...
      if (settings.isOpenSearchSecurityEnabled()) {
        if (settings.isOpenSearchJWTEnabled() && project != null && (user != null || runAsDataOwner)) {
          String token = runAsDataOwner ?
              openSearchJWTController.createTokenForELKAsDataOwner(project) :
              openSearchJWTController.createTokenForELK(user, project);
          httpRequest.setHeader(HttpHeaders.AUTHORIZATION, "Bearer " + token);
        } else {
          String userPass =
              settings.getOpenSearchAdminUser() + ":" +
