 ...
 */
public class HopsworksLatestVersionAnalyzer extends LatestVersionAnalyzer {

  private static final Logger LOGGER = Logger.getLogger(
    HopsworksLatestVersionAnalyzer.class.getName());
  ...
  public void setLatestVersion(String currentHopsworksVersion, HashMap<String, List<LibraryVersionDTO>> versions) {
    ...
    if(versions != null && !versions.isEmpty() && versions.containsKey(this.getLibrary())) {
      ...
      List<LibraryVersionDTO> currentVersionSearchHits = searchVersionHits
        .stream().filter(version -> version.getVersion().startsWith(hopsworksLibVersionPrefix))
