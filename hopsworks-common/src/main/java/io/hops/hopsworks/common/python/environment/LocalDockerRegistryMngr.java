...
public class LocalDockerRegistryMngr implements DockerRegistryMngr {
  ...
  public void gc() throws IOException, ServiceException {
    ...
    try {
      for (CondaCommands cc : condaCommandsRemove) {
        ...
        if (!projectDockerImage.equals(settings.getBaseDockerImagePythonName()) &&
          ...
          try {
            ...
            URI registryURL =
              URI.create("https://" + projectUtils.getRegistryURL() + "/v2/" + projectDockerImageNoTags + "/tags" +
                "/list");
            ...
            String httpResp = httpClient.execute(host, request, httpResponse -> {
              if (httpResponse.getStatusLine().getStatusCode() >= 400) {
                throw new IOException(
                  "Could not fetch tags from registry: " + httpResponse.getStatusLine().toString());
              }
            ...
            if(respJson.has("tags") && respJson.get("tags") != "null") {
              ...
              for (int i = 0; i < tagsJSON.length(); i++) {
                ...
                if (processResult.getExitCode() != 0) {
                  throw new IOException("Could not delete the docker image. Exit code: " + processResult.getExitCode()
                ...
                dockerImagesGC(projectUtils.getRegistryURL() + "/" + projectDockerImageNoTags + ":" + tag);
              }
            }
            ...
            try {
              commandsController.updateCondaCommandStatus(cc.getId(), CondaStatus.FAILED, cc.getArg(), cc.getOp(),
                "Could not complete docker registry cleanup: " + ex.getMessage());
      ...
      if (!condaCommandsRemove.isEmpty()) {
        ...
        ProcessResult processResult = osProcessExecutor.execute(processDescriptor);
        if (processResult.getExitCode() != 0) {
          LOG.log(Level.WARNING, "Could not delete the docker image. Exit code: " + processResult.getExitCode()
            + " out: " + processResult.getStdout() + "\n err: " + processResult.getStderr());
        }
      }
    }
