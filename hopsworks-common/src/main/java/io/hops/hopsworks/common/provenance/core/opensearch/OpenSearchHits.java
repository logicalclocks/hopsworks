...
public class OpenSearchHits {
  /**
   * Merge opensearch parsed element R into a container S
   */
  ...
  /**
   * Handler for processing opensearch raw results into the actual container result S, orchestrating the Merger & Parser
   */
  ...
  public static <R, S> Handler<R, S> handlerBasic(Parser<R> parser, S state, Merger<R, S> stateMerger) {
    return new OpenSearchHitsHandlerImpl<>(parser, state, stateMerger);
  }
  
  /**
   * Utility method that uses a simple List Merger(that simply accumulates all items)
   */
  public static <R> Handler<R, List<R>> handlerAddToList(OpenSearchHits.Parser<R> parser) {
    ListMerger<R> simpleListAcc = (R item, List<R> s) -> {
      s.add(item);
      return Try.apply(() -> s);
    };
    return new OpenSearchHitsHandlerImpl<>(parser, new ArrayList<R>(), simpleListAcc);
  }
  
  /**
   * Utility method that uses a simple Set Merger(that simply accumulates all items)
   */
  public static <R> Handler<R, Set<R>> handlerAddToSet(OpenSearchHits.Parser<R> parser) {
    SetMerger<R> simpleListAcc = (R item, Set<R> s) -> {
      s.add(item);
      return Try.apply(() -> s);
    };
    return new OpenSearchHitsHandlerImpl<>(parser, new HashSet<R>(), simpleListAcc);
  }
}
