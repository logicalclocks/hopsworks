...
@Stateless
public class JobAlertsFacade extends AbstractFacade<JobAlert> {
  
  @PersistenceContext(unitName = "kthfsPU")
  private EntityManager em;
  
  public JobAlertsFacade() {
    super(JobAlert.class);
  }
  ...
  public JobAlert findByJobAndId(Jobs jobs, Integer id) {
    TypedQuery<JobAlert> query = em.createNamedQuery("JobAlert.findByIdAndJob", JobAlert.class);
    query.setParameter("jobId", jobs).setParameter("id", id);
    try {
  ...
  public JobAlert findByJobAndStatus(Jobs jobs, JobAlertStatus jobAlertStatus) {
    TypedQuery<JobAlert> query = em.createNamedQuery("JobAlert.findByJobAndStatus", JobAlert.class);
    query.setParameter("jobId", jobs).setParameter("status", jobAlertStatus);
    try {
      return query.getSingleResult();
    } catch (NoResultException e) {
      return null;
  ...
  public CollectionInfo findAllJobAlerts(Integer offset, Integer limit, Set<? extends FilterBy> filter,
      Set<? extends SortBy> sort, Jobs jobs) {
    String queryStr = buildQuery("SELECT a FROM JobAlert a ", filter, sort, "a.jobId = :job ");
    String queryCountStr = buildQuery("SELECT COUNT(a.id) FROM JobAlert a ", filter, sort, "a.jobId = :job ");
    Query query = em.createQuery(queryStr, Activity.class).setParameter("job", jobs);
    Query queryCount = em.createQuery(queryCountStr, Activity.class).setParameter("job", jobs);
  ...
  private void setFilterQuery(AbstractFacade.FilterBy filterBy, Query q) {
    switch (JobAlertsFacade.Filters.valueOf(filterBy.getValue())) {
      case TYPE:
        q.setParameter(filterBy.getField(), getEnumValues(filterBy, AlertType.class));
        break;
      case STATUS:
        q.setParameter(filterBy.getField(), getEnumValues(filterBy, JobAlertStatus.class));
        break;
      case SEVERITY:
  ...
  public enum Filters {
    ...
    STATUS("STATUS", "a.status IN :status ", "status", JobAlertStatus.FINISHED.toString()),
