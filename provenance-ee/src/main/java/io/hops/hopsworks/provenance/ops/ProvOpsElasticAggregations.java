...
public class ProvOpsElasticAggregations {
  ...
  public ElasticAggregationParser<?, ProvenanceException> getAggregationParser(ProvOpsAggregations agg)
  ...
  private ElasticAggregationParser<ProvOpsDTO, ProvenanceException> artifactOpsAppAggregationParser() {
          ...
          for (Provenance.FileOps fileOp : appAggregationOpsList) {
            ...
            if (createFilter != null) {
              ...
              if (opHits.getHits().getHits().length == 1) {
                BasicElasticHit hit = BasicElasticHit.instance(opHits.getHits().getAt(0));
  ...
  private ElasticAggregationParser<ProvOpsDTO, ProvenanceException> appIdAggregationParser() {
    return (Aggregations aggregations) -> {
      ...
      for (Terms.Bucket appBucket : apps.getBuckets()) {
        ...
        if (recentOpTopHit.getHits().getHits().length == 1) {
          ...
          BasicElasticHit hit = BasicElasticHit.instance(recentOpTopHit.getHits().getAt(0));
